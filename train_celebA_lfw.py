...
from model import NetworkCELEBALFW, NetworkCIFAR
...
parser = argparse.ArgumentParser("LFW and CELEB_A")
...
def infer_auc(valid_queue, model):
    ...
    for step, (pair, target) in enumerate(valid_queue):
        ...
        image_a = Variable(image_a, volatile=True).cuda()
        image_b = Variable(image_b, volatile=True).cuda()
