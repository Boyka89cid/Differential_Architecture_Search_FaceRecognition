...
class ContrastiveCenterLoss(nn.Module):
    ...
    def forward(self, y, hidden):
        ...
        # print(self.centers)
        expanded_centers = self.centers.expand(batch_size, -1, -1).half()
        ...
        epsilon = 1e-6
        # if torch.isnan(expanded_hidden.sum()):
        # loss = (self.lambda_c / 2.0 / batch_size) * intra_distances / \
        #        (inter_distances + epsilon) / 0.1
        if torch.isinf(inter_distances):
            inter_distances = torch.cuda.HalfTensor(1).fill_(1e4)
        if torch.isinf(intra_distances):
            intra_distances = torch.cuda.HalfTensor(1).fill_(1e4)
        loss = (self.lambda_c / 2.0 / batch_size) * intra_distances / \
               (inter_distances + epsilon) / 1e-4
        # logger.info('{},{},{}'.format(
        #     intra_distances.data[0], inter_distances.data[0], loss.data[0]))
        if torch.isnan(expanded_hidden.sum()):
            print(hidden)
            exit()
        # print(expanded_hidden.sum(), expanded_centers.sum(), loss, intra_distances, inter_distances)
        return loss
