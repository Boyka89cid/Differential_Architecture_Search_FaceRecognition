...
def main():
    ...
    if 'DARTS' in args.arch:
        ...
        if args.dataset == 'cifar':
            ...
            model = NetworkCELEBALFW(args.init_channels, numclasses, args.layers, genotype, args.drop_path_prob,
                                     args.ccl_loss)
...
def infer_auc(valid_queue, model):
    ...
    for step, (pair, target) in enumerate(valid_queue):
        ...
        image_a = Variable(image_a).cuda()
        image_b = Variable(image_b).cuda()
        ...
        # image_a_features = model(image_a)
        # image_b_features = model(image_b)
